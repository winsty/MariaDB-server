...
public:
  ...
  void merge_range(range_t range, range_t prev_range)
  {
  ...
  void split_range(range_t range, uint32_t value)
  {
    ...
    ranges.erase(range);
    range_t split1{split1_start_val, value - 1};
    range_t split2{value + 1, split2_end_val};
  ...
  void remove_within_range(range_t range, uint32_t value)
  {
...
insert_range:
  ...
  void remove_value(uint32_t value)
  {
    ...
    if (range == ranges.end())
    {
      /* Element could be in last range */
      return remove_within_range(*rlast, value);
    }
    ...
    if (range->first > value && range != ranges.begin())
    {
      ...
      return remove_within_range(*prev_last, value);
    }
    else return remove_within_range(*range, value);
 }
 /** Add the value within the existing range
 @param[in]	range	range to be modified
 @param[in]	value	value to be added */
 bool add_within_range(range_t range, uint32_t value)
 {
...
new_range:
   ...
   {
     /* last range */
     if (!add_within_range(*rlast, new_range.first))
       goto new_range;
     return;
   }
   ...
   {
     /* First range */
     if (!add_within_range(*r_offset, new_range.first))
       goto new_range;
     return;
   }

   /* Change starting of the existing range */
   if (r_offset->first - 1 == new_range.first)
   {
     add_within_range(*r_offset, new_range.first);
     if (r_offset != ranges.begin())
     {
   ...
   else
   {
     ...
     if (!add_within_range(*prev_last, new_range.first))
       goto new_range;
   }
  }
